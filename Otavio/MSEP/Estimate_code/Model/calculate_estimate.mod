...
% Summer Job - XP Asset 
% 1Q de 2022
% Dúvidas/sugestões: otavio1teixeira@gmail.com
%-----------------------------------------------
...
% estimate the model in 2 steps:
% 1) recover mode using mode_compute = 1
% 2) use mode from step 1 and set mh_replic = 200k.

estimation(datafile = data_nueva_MEP_reducida,
           % mode_file = MSEP_mode,
           nobs = 74, % 74 = até dez/2019, pre-pandemia.
           plot_priors = 0,
           % mode_check,
           prefilter = 0,
           mh_nblocks = 1,
           mh_drop = 0.25,
           mh_jscale = 0.21, % This option must be tuned to obtain, ideally, an acceptance ratio of 25%-33%.
           mode_compute = 1, % 1
           % tex, 
           mh_replic = 0, smoother) u rn; % paper uses 200000 replics, we estimated the model also using 200k replics. Set 1k just to verify if everything is ok
...
stoch_simul(periods=0, irf=60, order=1, nograph);
